...
- Autorouting auto-setup: a curated per-model tier-label map in `packages/coding-agent` (CI-forced against the committed model catalog with a landing baseline skip list), a pure clock-free chain generator that materializes cross-provider `fast`/`balanced`/`strong` fallback chains from a declared provider order, a `/model` smart-routing panel offering atomic preview/apply/refresh/clear with provenance and a hand-edit guard, bounded autorouting-preflight retry with typed transport classification (max three unique candidates, pre-start transient failures only, no mid-run failover), staged session and attempt-scoped artifact isolation so failed attempts leave zero durable residue, and full routing evidence covering skips, attempts, and terminal outcomes. Declared providers are the sole priority channel, an optional model list only filters eligibility, and credentials never influence generated routing.
